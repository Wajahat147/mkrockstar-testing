...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    const [prodRes, custRes, ordRes, dealsRes] = await Promise.all([
      dbClient.from('products').select('*'),
    ...
    if (ordRes.data) {
      orders = ordRes.data.map(o => {
        if(customerMap[o.customer_id]) {
          ...
          if (customerMap[o.customer_id].spent > 500) customerMap[o.customer_id].tier = 'VIP';
          else if (customerMap[o.customer_id].orders > 1) customerMap[o.customer_id].tier = 'Regular';
        }
...
function renderDashboard() {
  ...
  document.getElementById('stat-sales').textContent = '$' + (total/1000).toFixed(1)+'K';
...
function handleDealImageFile(input) {
  ...
  if(file.size > 10 * 1024 * 1024) { showToast('Image must be under 10MB','error'); return; }
  const reader = new FileReader();
  reader.onload = e => setDealImagePreview(e.target.result);
  reader.readAsDataURL(file);
}
...
async function saveProduct(e) {
  ...
  try {
    if(id) {
      ...
      dbData.id = Math.floor(Date.now() / 1000); // Provide explicit ID that fits in INT4
...
function renderDeals() {
  ...
  grid.innerHTML = deals.map(d => `
    <div class="product-card flex flex-col" style="overflow:hidden; ${!d.is_active ? 'opacity:0.6;' : ''}">
      <div style="height:200px;overflow:hidden;background:rgba(255,255,255,0.04);position:relative;">
        <img src="${d.image_url || 'https://via.placeholder.com/400x500?text=No+Image'}" alt="${d.title}" style="width:100%;height:100%;object-fit:cover;"/>
        <div style="position:absolute;top:8px;right:8px;">
